...
状态：代码位于 `codex/update-xfixes-security`，构建来源 `1e4a1cf5149df2b5da48d2571b08f2bc597a538b`。本计划和候选报告进入主线便于跟踪，不表示修复代码已经合入。
...
- [ ] 接续当前 master 的依赖基线关系和维护文档，复跑全部必要自动测试。
- [ ] 取得维护者对此 XFIXES 候选的实际使用确认，记录范围和未验证场景。
- [ ] 合入 master 后复核源码、测试和运行目录，并更新状态；upstream 仍保存依赖来源集合。
