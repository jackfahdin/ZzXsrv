...
2026-09-10 更新：维护者已确认实际使用的是 `vcxsrv-input-20260909-2110`，基本操作和多窗口使用正常；剪贴板、OpenGL 未验证。已打开菜单跨屏后停留原屏的问题已记为 [GUI-001](../maintenance/COMPATIBILITY.md#gui-001主窗口跨屏移动后已打开菜单留在原屏幕)，按维护者要求暂缓排查。随后已恢复补丁、适配当前目录，完整构建、73 项测试、运行验证与独立审查通过，已完成主线整合；见[整合报告](../validation/2026-09-10-xserver-input-integration.md)。下文保留 2026-09-09 的历史检查点。
...
- [x] 取得本候选自身的实际使用反馈（2026-09-10）；记录正常场景、未验证范围和暂缓排查的 GUI-001，不沿用旧 GLX/XFIXES 反馈。
- [x] 在当前源码布局下恢复补丁和测试、更新来源记录，完整构建、73 项测试及运行验证通过后整合至线性 master；保留 GUI-001 和未验证范围。
