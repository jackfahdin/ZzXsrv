...
- 新增 5 项真实静态库消费者检查：版本、非 C locale 下小数字号解析/回写、常量反查、通用字体属性往返、显式配置默认值。旧 2.16.0 库下 5 项失败；新版组件下 5 项通过。旧版新 API 缺失属于升级差异，不作为旧版故障。
...
- 现有 FreeType 缓存消费者增加格式 12、缓存内按位编码的 2.18.3 版本和重载不改写缓存的断言。另行验证真实旧、新缓存共存。
