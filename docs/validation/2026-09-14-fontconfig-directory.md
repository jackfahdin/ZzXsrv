...
状态：2026-09-15 已取得本版本“没问题，继续”的实际反馈，并快进合入 master；合入前后各 370 项测试通过、0 跳过，未 push。缓存创建及重载均为 2 条记录，独立运行检查 PASS。构建曾中断并留下损坏中间产物，已按阶段重建恢复，经过与失败记录见下文。
...
2026-09-15，维护者针对交付的新目录反馈“没问题，继续”，记录为本版本实际使用正常。未提供逐项操作结果，不由此补造字体／字号切换、传统字体、OpenGL 或双向复制的通过记录。详细边界见 [兼容性记录](../maintenance/COMPATIBILITY.md)。

master 从 `1bb897d4a368521e5aaab9e11c686c9c3ccebc20` 快进至 `be360d995fe2df46056f5f3346f37ce90381ca95`，保留修复、回归测试及候选文档提交，没有 merge commit。

合入前后各 370 项测试通过、0 跳过，耗时分别为 40.616 秒和 43.063 秒。两次均在保留的工作区运行，使用与修复匹配的本机构建依赖及运行目录；合入后核实工作区与 master 指向同一提交且文件无差异。测试结束后只补充整合文档，不改变生产代码；本轮没有重跑完整构建，构建来源仍为 `cd444aa641d50355c0d7b47f4a7167b78e313e45`。

重新核对交付清单，5129 个运行文件的集合与 SHA-256 均未改变。继续使用 `D:/File/Program/GitHub/zzxsrv-fontconfig-20260914/dist/x64/Release/xlaunch.exe`；主目录源码更新不会自动重编译其旧 dist。运行目录、临时分支和工作区登记保留，未 push。

整合证据保存在主目录 `.local-validation/fontconfig-integration-20260915/`，包含合入前后测试日志和 `verification.json`。本次目录枚举修复已完成整合，其他组件维护及下述验证边界继续保留。
