...
[R13 xserver 上游补充批次（收官）](2026-09-17-r13-xserver.md)：按 R11 评估的 B/C 档精选 6 个上游提交全部干净 cherry-pick——xkb 成对序列化修复（`bdc7eb8f`+`5ebf0a9d`，按上游顺序成对应用）、render gradient 错误路径泄漏（`123f50ac`）、rootless Glyphs damage box 并集计算错误（`09eb9aa3`，multiwindow 文本重绘）、2 项 xserver 许可证文本同步（作用于 `src/xorg-server/COPYING`）。其余 B/C 档 16 个评估为不适用或收益不足，明确不再导入。完整 x64 Release 构建通过（5155 个交付文件），452 项回归通过、0 跳过（针对新构建运行目录）。编译源码 `cda929acaafb96d533ec0beb209635ea356458c8`；候选待实测验收，未合入或推送。

[R12 xserver 上游安全补丁批次](2026-09-17-r12-xserver.md)：已于 2026-09-17 取得维护者"测试通过"的反馈并快进合入 master（未提供逐项明细，未确认范围保留）。按 R11 评估的 A 档从 server-21.1-branch cherry-pick 19 个安全/崩溃修复（render glyph UAF、dix/colormap 越界读、glx 负长度、dix FollowKeyboard 崩溃与反向 BUG_RETURN、Xi 六个空指针/未初始化值、xtest/xres 空指针、sync 三连含一个 UAF），每个补丁一个本地提交并保留上游署名；17 个干净套用，`b171669dc0c9` 与 `976ef43f95a3` 因本地 `X11_RESTYPE_NONE` 改名手工适配。完整 x64 Release 构建通过（5155 个交付文件），合入前后各 452 项回归通过、0 跳过。编译源码 `2fe3f05341be968ede39821bbc6f8abb93dce83f`；已验收入口 `D:/File/Program/GitHub/zzxsrv-r12-xserver-20260917/dist/x64/Release/xlaunch.exe`。分支、工作区与运行目录保留，未推送。B/C 档未导入。

[R9.9 嵌套片段归属澄清](2026-09-17-r9-fragments.md)：已于 2026-09-17 取得维护者验收并快进合入 master。SOURCES 清单 7 个"未知/未固定"嵌套片段逐一以上游证据澄清：libregex 定位为 gnulib 2014-01-01..03-05 提交窗口（5 个文件逐字节一致，glibc 全部 tag 已排除）；libwinmain 证实为 VcXsrv/marha 项目原创（引入提交 `17cd2280a71d`，PuTTY 来源已排除）；dxtn 定位为 libtxc_dxtn 20070518 快照经 O3D trunk 转引（1.0/1.0.1 已排除，含一处 VcXsrv 本地补丁）；dirent.h=dirent 1.10（Wayback 官方 zip 比对 + 3 处本地补丁）；inttypes.h/stdint.h=msinttypes r26；include/xcb utility 头逐文件定位（xcb-util 0.4.1、xcb-util-image 0.4.1、xcb-util-wm 0.4.2 等）；zlib 条目修正为 R8 实际导入的 1.3.2（官方归档逐文件复核）。仅更新 SOURCES 清单，无产品代码改动；合入后 452 项回归通过、0 跳过。分支与工作区保留，未推送。R9 系列至此全部完成。

[R11 xserver 上游未合入提交适用性评估](2026-09-17-r11-xserver-applicability.md)：已于 2026-09-17 取得维护者验收并快进合入 master。对 R9.8 筛出的 server-21.1-branch 104 个本地未合入提交逐 commit 判定（核对上游 diff、本地 mhmake 构建覆盖、漏洞模式在场性与 fork 冲突风险）——A 建议合入 19 个（glyph UAF、colormap 越界读、glx 负长度、Xi/Xext/dix 空指针等，模式均已在本地确认在场）、B 可选 15 个、C 需功能评估 5 个、D 不适用 65 个（glamor/xfree86/xquartz/发布与构建系统等不构建部分）。纯评估不导入代码，无产品改动；合入后 452 项回归通过、0 跳过。分支与工作区保留，未推送。A 档是否另立 R12 补丁整合由维护者决策。

[R9.8 xserver 来源补证](2026-09-17-r9-xserver.md)：已于 2026-09-17 取得维护者验收并快进合入 master。xserver 记录固定提交 `af86c5cbc2475b410a2edeb0482ac58309296adb` 已在官方仓库核验存在（位于通往 21.2 的 master 线，非 21.1 分支）。`src/xorg-server` 本体 1674 个文件逐文件比对：1314 个逐字节相同，263 个差异全部归类（21 个已登记补丁文件、65 个 hw/xwin fork 改动、95 个 MSVC/Win32 适配、112 个其他继承适配，抽样核对无不可解释差异）；39 个已登记上游补丁触及的文件全部落在差异集内，零遗漏。纯来源补证，无产品代码改动，不需要重建；合入后 452 项回归通过、0 跳过。分支与工作区保留，未推送。上游 21.1 分支尚有 104 个未合入提交（含安全相关候选），另行评估。

[R9.7 四组件来源补证](2026-09-17-r9-apps.md)：已于 2026-09-17 取得维护者验收并快进合入 master。xkbcomp、xwininfo、mkfontscale、xkeyboard-config 的记录固定提交均已在官方仓库核验存在，并完成逐文件比对（明细见各组件 `source-audit.json`）；差异全部为继承适配与本地构建文件，无不可解释改动。xkbcomp 的继承记录修正为 R8 导入的官方 1.5.0（tag 提交已核验）。纯来源补证，无产品代码改动；合入后 452 项回归通过、0 跳过。分支与工作区保留，未推送。

[R9.6 PuTTY 0.85](2026-09-16-r9-putty.md)：已于 2026-09-16 取得维护者验收（常规场景通过；SSH 转发真实场景未实测，边界保留）并快进合入 master。`tools/plink` 升级为官方 0.85 发布归档（SHA-256 与官方 clearsigned 校验一致、GPG 签名验证通过、与 git tag 交叉核对；含 0.84/0.85 安全修复）。完整 x64 Release 构建、合入前后各 452 项回归（0 跳过，含 3 项 plink 检查）、候选独立运行检查通过；6 个继承适配保留 5 个、1 个因上游已修复丢弃。编译源码 `e0cad4dbca888c12b88629e76b74726d39642f8d`；5152 个交付文件及摘要不变。分支、工作区与新旧运行目录保留，未推送。

[R9.5 PuTTY 来源补证](2026-09-16-r9-plink.md)：已于 2026-09-16 取得维护者验收并快进合入 master。上游固定提交 `50e360fc7474898b4de0e3533aff50857f2b3289`（`0.83-24-g50e360fc`）已在官方仓库核验存在，与 LATEST.VER 的 0.83 一致。682 个本地跟踪文件逐文件比对：669 个与上游逐字节相同、6 个为继承适配、7 个为本地构建集成，无不可解释差异，明细见 `tools/plink/source-audit.json`。无产品代码改动；合入后 449 项回归通过、0 跳过。上游 0.84/0.85 含多项安全修复，PuTTY 升级另立 R9.6，来源评估结论为继续采用官方上游而非中文 fork（见报告）。
