...
[R9.7 四组件来源补证](2026-09-17-r9-apps.md)：xkbcomp、xwininfo、mkfontscale、xkeyboard-config 的记录固定提交均已在官方仓库核验存在，并完成逐文件比对（明细见各组件 `source-audit.json`）；差异全部为继承适配与本地构建文件，无不可解释改动。xkbcomp 的继承记录修正为 R8 导入的官方 1.5.0（tag 提交已核验）。纯来源补证，无产品代码改动，不需要重建；候选待验收，未合入或推送。
