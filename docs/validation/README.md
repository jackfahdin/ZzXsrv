...
[R9.5 PuTTY 来源补证](2026-09-16-r9-plink.md)：上游固定提交 `50e360fc7474898b4de0e3533aff50857f2b3289`（`0.83-24-g50e360fc`）已在官方仓库核验存在，与 LATEST.VER 的 0.83 一致。682 个本地跟踪文件逐文件比对：669 个与上游逐字节相同、6 个为继承适配、7 个为本地构建集成，无不可解释差异，明细见 `tools/plink/source-audit.json`。本轮无产品代码改动，不需要完整重建；候选待验收，未合入或推送；R9.4 仍为最新已验收版本。上游 0.84/0.85 含多项安全修复，是否升级 PuTTY 属产品决策，另行立项。
