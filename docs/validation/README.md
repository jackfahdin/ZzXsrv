...
[R9.9 嵌套片段归属澄清](2026-09-17-r9-fragments.md)：SOURCES 清单 7 个"未知/未固定"嵌套片段逐一以上游证据澄清：libregex 定位为 gnulib 2014-01-01..03-05 提交窗口（5 个文件逐字节一致，glibc 全部 tag 已排除）；libwinmain 证实为 VcXsrv/marha 项目原创（引入提交 `17cd2280a71d`，PuTTY 来源已排除）；dxtn 定位为 libtxc_dxtn 20070518 快照经 O3D trunk 转引（1.0/1.0.1 已排除，含一处 VcXsrv 本地补丁）；dirent.h=dirent 1.10（Wayback 官方 zip 比对 + 3 处本地补丁）；inttypes.h/stdint.h=msinttypes r26；include/xcb utility 头逐文件定位（xcb-util 0.4.1、xcb-util-image 0.4.1、xcb-util-wm 0.4.2 等）；zlib 条目修正为 R8 实际导入的 1.3.2（官方归档逐文件复核）。仅更新 SOURCES 清单，无产品代码改动，不需要重建；候选待验收，未合入或推送。
