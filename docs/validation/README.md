...
[R9.6 PuTTY 0.85](2026-09-16-r9-putty.md)：`tools/plink` 升级为官方 0.85 发布归档（SHA-256 与官方 clearsigned 校验一致、GPG 签名验证通过、与 git tag 交叉核对）。完整 x64 Release 构建、452 项回归（0 跳过，含 3 项新增 plink 检查）、红绿冒烟与候选独立运行检查通过；6 个继承适配保留 5 个、1 个因上游已修复而丢弃。编译源码 `e0cad4dbca888c12b88629e76b74726d39642f8d`。真实 SSH 转发场景未实测；候选待验收，未合入或推送；R9.4 的 dist 仍为最新已验收运行版本。
