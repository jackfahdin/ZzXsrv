...
[R12 xserver 上游安全补丁批次](2026-09-17-r12-xserver.md)：按 R11 评估的 A 档从 server-21.1-branch cherry-pick 19 个安全/崩溃修复（render glyph UAF、dix/colormap 越界读、glx 负长度、dix FollowKeyboard 崩溃与反向 BUG_RETURN、Xi 六个空指针/未初始化值、xtest/xres 空指针、sync 三连含一个 UAF），每个补丁一个本地提交并保留上游署名；17 个干净套用，`b171669dc0c9` 与 `976ef43f95a3` 因本地 `X11_RESTYPE_NONE` 改名手工适配。完整 x64 Release 构建通过（5155 个交付文件），452 项回归通过、0 跳过（针对新构建运行目录）。编译源码 `2fe3f05341be968ede39821bbc6f8abb93dce83f`；候选待实测验收，未合入或推送。B/C 档未导入。
