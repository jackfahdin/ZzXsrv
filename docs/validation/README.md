...
[R11 xserver 上游未合入提交适用性评估](2026-09-17-r11-xserver-applicability.md)：对 R9.8 筛出的 server-21.1-branch 104 个本地未合入提交逐 commit 判定（核对上游 diff、本地 mhmake 构建覆盖、漏洞模式在场性与 fork 冲突风险）——A 建议合入 19 个（glyph UAF、colormap 越界读、glx 负长度、Xi/Xext/dix 空指针等，模式均已在本地确认在场）、B 可选 15 个、C 需功能评估 5 个、D 不适用 65 个（glamor/xfree86/xquartz/发布与构建系统等不构建部分）。纯评估不导入代码，无产品改动，不需要重建；候选待验收，未合入或推送。
