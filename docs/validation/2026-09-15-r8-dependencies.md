...
组件提交分别为 `34b2823f3`（xkbcomp）、`644e55366`（libXpm）、`1856e4ed3`（zlib）；测试、打包和来源记录在 `4f8ddf84ae9f7e1da90a884577c31fa703f04725`。提交后于 15:40:48–15:41:57 执行 `Server -Jobs 16`，包含 Portable，退出 0；该轮发生部分 Mesa 对象重建和链接，日志保留既有重复对象警告，不宣称无警告构建。

| 检查 | 结果 |
| --- | --- |
| 提交后完整自动回归 | 420 项通过、0 跳过，46.158 秒；使用 R8 工作区实际对象、库及交付 DLL |
| 独立运行检查 | PASS，1.397 秒；33 个 EXE/DLL 架构与依赖、版本、带认证启动、根窗口查询和退出清理 |
| 最终运行目录 | 5143 个实际文件，33 个 EXE/DLL；比 R7 增加 7 个许可证/来源说明，没有新增运行 DLL。打包器打印的 5146 含重复拷贝项 |
| 产物对应 | 交付 zlib1.dll、xkbcomp.exe 与本轮生产输出摘要相同；上游 zlib 示例使用的 DLL 与交付 DLL 相同 |
| 旧版本保护 | R7 的 5136 个文件及摘要全部不变；主线仍为 `20901884259bacfe34b9ca435c81021862800a5a`，工作区干净 |
| 来源与提交内容 | 467 个官方归档文件、474 个导入/本地构建文件摘要及路径核验通过，另列摘要、补丁和来源说明；构建输入摘要未变 |
