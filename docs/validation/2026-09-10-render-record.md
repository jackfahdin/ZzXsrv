# R3 RENDER/RECORD 候选验证

日期：2026-09-10。状态：重启后分阶段恢复构建完成，97 项测试、独立运行验证和审查通过；本组人工使用反馈尚未取得，候选尚未合入 master。旧文件比较发现主仓库 dist 的 16 项差异，已保留并记录，未声称所有旧文件均未变化。

## 范围与来源

维护者要求开始 R3 的 RENDER/RECORD 修复。起点为 `1bc06775c03e128168064d931f63667d1bc95598`，候选分支为 `codex/render-record-20260910`，工作目录为 `D:/File/Program/GitHub/zzxsrv-render-record-20260910`。保留 GLX、XFIXES 和输入处理修复，GUI-001 按此前要求暂缓排查。
...
候选运行目录：`D:/File/Program/GitHub/zzxsrv-render-record-20260910/dist/x64/Release`。主目录和此前输入候选不会自动更新；此前版本的“正常”反馈不作为本组人工验收。

维护者使用本组候选后，可按日常方式检查程序开关和再次打开、实际 X11 应用的窗口/菜单/键盘与多窗口操作、鼠标形状和忙碌光标是否异常。普通 Windows 原生 gitk 的运行不能验证 X Server。暂无远程 Linux 环境的项目应继续标为未验证，无须专门运行异常请求或漏洞触发程序。

本组通过自动检查后保留为“候选待验收”。取得本组实际使用确认后按快进方式进入线性 master，再解除临时工作区登记、清理临时分支并保留运行目录。仅作本地提交，不 push。R3 历史 21 项中，输入 2 项已经主线整合；RENDER/RECORD 2 项在候选，另外 17 项尚待实施。GUI-001、双向剪贴板和 OpenGL 等既有未验证范围保持原记录。
