...
状态：候选完整构建与正常兼容性验证完成，363 项测试通过，0 失败、0 跳过；待维护者实际使用确认，未合入 master，未 push。异常输入回归未执行，Fontconfig 既有缓存重复问题单独跟踪。
...
R3 的实际反馈仅适用于旧目录：启动、gitk、中文显示和重启正常；传统字体、OpenGL、双向复制未测试。R4 尚待新候选实际使用反馈。GUI-001 已关闭，不重新开展。

## 候选使用与人工验收

新运行入口：`D:/File/Program/GitHub/zzxsrv-fonts-20260914/dist/x64/Release/xlaunch.exe`。按平时流程退出旧服务器后，从这个入口启动；避免把旧目录的成功反馈算到新版本。

1. 用原连接方式验证启动、gitk 的滚动/选择/菜单、中文显示和退出重启。
2. 对日常应用已有字体设置，切换等宽/非等宽字体及字号，观察缺字、截断、错位或崩溃。
3. 如 Linux 端已有 xterm，可运行 `xterm -fn 7x13` 补充传统字体人工检查；沿用现有 DISPLAY/认证，不修改连接安全设置。未安装则继续记为未测。

建议反馈格式：`R4 新目录：启动正常/异常；gitk 正常/异常；中文正常/异常；字体/字号切换正常/未测；传统字体正常/未测；重启正常/异常。`

主线仍保留上一组已验收版本。本候选正常验证已经完成，但异常输入回归缺口和人工验收仍明确保留；不自动宣称 R4 全场景完成。
