"""Run real RENDER animated cursor code under MSVC AddressSanitizer.
...
class RenderAnimCursorTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        compiler = shutil.which("cl.exe")
        if not compiler:
            raise RuntimeError("cl.exe missing: run from an x64 MSVC developer environment")
        cls.directory = tempfile.TemporaryDirectory(prefix="vcxsrv_render_")
        cls.addClassCleanup(cls.directory.cleanup)
        work = Path(cls.directory.name)
        cls.exe = work / "render_animcursor.exe"
        ...
        command = [compiler, "/nologo", "/std:c11", "/Od", "/Gy", "/Gw", "/Zc:inline", "/MD", "/Zi",
                   ...
                   *["/D" + d for d in defines],
                   str(ROOT / "tools/tests/native/render_animcursor.c"),
    ...
    def test_negative_public(self):
        self.run_case("negative-public")

    def test_one_public(self):
        self.run_case("one-public")

    def test_two_public(self):
        self.run_case("two-public")

    def test_valid_protocol(self):
        self.run_case("valid-protocol")

    def test_allocation_error(self):
        self.run_case("allocation-error")

    def test_lookup_error(self):
        self.run_case("lookup-error")

    def test_security_error(self):
        self.run_case("security-error")
